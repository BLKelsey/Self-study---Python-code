# Please write a function named formatted, which takes a list of floating point numbers as its argument.
# The function returns a new list, which contains each element of the original list in string format, rounded to two decimal points.
# The order of the items in the list should remain unchanged.
#
# Hint: use f-strings to format the floating point numbers into suitable strings.

def formatted(mylist):
    return [f"{num:.2f}" for num in mylist]    # make a NEW list where each number is turned into a string with 2 decimal places


# Example usage
mylist = [1.234, 0.3333, 0.11111, 3.446]
new_list = formatted(mylist)

print()
print(f'Original: {mylist}')
print(f'Formatted: {new_list}')




